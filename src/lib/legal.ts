import { site, giveaway } from "@/lib/config";
import { formatCents } from "@/lib/entries";

const drawDate = new Date(giveaway.drawDateIso).toLocaleDateString("en-US", {
  month: "long",
  day: "numeric",
  year: "numeric",
});

export type LegalPage = { title: string; body: string };

export const legalPages: Record<string, LegalPage> = {
  rules: {
    title: "Official Rules",
    body: `# ${site.name} Giveaway ${giveaway.id} — Official Rules

**NO PURCHASE NECESSARY TO ENTER OR WIN. A PURCHASE WILL NOT INCREASE YOUR CHANCES OF WINNING. VOID WHERE PROHIBITED.**

> ⚠️ TEMPLATE NOTICE: These rules are a working draft. They MUST be reviewed and finalized by a licensed sweepstakes attorney before this promotion launches. See LAUNCH.md.

## 1. Sponsor
${site.legalName}, [ADDRESS — required]. Contact: ${site.supportEmail}.

## 2. Eligibility
Open only to legal residents of the 48 contiguous United States and the District of Columbia who are 18 years of age or older at the time of entry. Void in New York, Florida, Rhode Island, and where prohibited by law. Employees, officers, and directors of the Sponsor, and members of their immediate families or households, are not eligible.

## 3. Promotion Period
The promotion begins upon public launch and ends at 11:59 PM ET on the day before the drawing. The drawing will be held on or about **${drawDate}**.

## 4. How to Enter
**(a) Purchase entry packages.** Entry packages are available on the Sponsor's website. Each package grants the number of sweepstakes entries stated at the time of purchase, recorded automatically against the purchaser's email address.

**(b) Free online entry (AMOE).** Visit the Free Entry page and complete the entry form with your full name, email, and mailing address to receive ${giveaway.amoeEntries.toLocaleString()} entries at no cost. Limit one (1) free online entry per person per day. Free entries have the same per-entry odds of winning as purchased entries.

**(c) Free mail-in entry.** Hand-print your full name, email address, mailing address, phone number, and date of birth on a 3"×5" card and mail it in a #10 envelope to: ${site.legalName}, Attn: ${giveaway.id} Free Entry, [ADDRESS — required]. Each mailed card received by the entry deadline grants ${giveaway.amoeEntries.toLocaleString()} entries. Limit one card per envelope, one mail-in entry per person per day. Photocopied or mechanically reproduced cards are void.

## 5. Prize
One (1) Grand Prize: a ${giveaway.car.year} ${giveaway.car.name} (approximate retail value ${formatCents(giveaway.arvCents)}) plus ${formatCents(giveaway.taxContributionCents)} cash toward taxes, **or**, at the winner's sole election, ${formatCents(giveaway.cashAlternativeCents)} cash in lieu of the vehicle. Vehicle transport within the 48 contiguous US states is included. Title transfer, registration, insurance, and all taxes are the winner's responsibility. Odds of winning depend on the total number of entries received.

## 6. Winner Selection & Notification
One potential winner will be selected in a random drawing from all valid entries received, conducted by [SWEEPSTAKES ADMINISTRATOR — required, e.g. a bonded independent administrator] on or about ${drawDate}. The potential winner will be notified by email and/or phone within 48 hours and must respond within seven (7) days and complete an affidavit of eligibility, liability release, and (where lawful) publicity release, plus IRS Form W-9. If a potential winner does not respond, is ineligible, or declines, an alternate will be drawn.

## 7. Taxes
The prize's fair market value is taxable income to the winner. Sponsor will issue IRS Form 1099-MISC. The ${formatCents(giveaway.taxContributionCents)} tax contribution is itself taxable.

## 8. Publicity
Except where prohibited, acceptance of the prize constitutes permission to use the winner's name, likeness, city/state, and prize information for promotional purposes without further compensation.

## 9. General Conditions
Sponsor may disqualify entries generated by scripts, bots, or fraud. Sponsor's decisions are final. By entering, entrants release Sponsor from all liability arising from the promotion or prize to the maximum extent permitted by law. Disputes will be resolved individually (no class actions) under the laws of [STATE — required].

## 10. Winners List
For the winner's name and city, email ${site.supportEmail} with the subject "${giveaway.id} Winner" within 60 days after the drawing.`,
  },

  terms: {
    title: "Terms of Service",
    body: `_Last updated: July 2026_

## Purchases
Entry package purchases are payments for sweepstakes entries in the active ${site.name} promotion, governed by the Official Rules. Entries are digital and delivered instantly to the email provided; purchases are final and non-refundable once entries are issued, except where required by law or where an order was made in error — email ${site.supportEmail} within 24 hours and we'll make it right.

## No purchase necessary
Purchasing does not improve per-entry odds. Free entry methods with identical per-entry odds are described in the Official Rules.

## Eligibility & accuracy
You are responsible for the accuracy of the name and email on your order — entries attach to that email. Entries made ineligible by false eligibility statements are void without refund.

## Intellectual property & trademarks
Site content belongs to ${site.legalName}. Vehicle manufacturer names and model designations are trademarks of their respective owners, used solely to identify the prize vehicle; this promotion is not affiliated with or endorsed by any vehicle manufacturer.

## Liability
To the maximum extent permitted by law, ${site.legalName}'s total liability related to the site or promotion is limited to the amount you paid for entries.

## Contact
${site.supportEmail}`,
  },

  privacy: {
    title: "Privacy Policy",
    body: `_Last updated: July 2026_

## What we collect
Order and entry information (name, email, mailing address), payment processing handled entirely by Stripe (we never see or store card numbers), and anonymous analytics (Google Analytics, Meta Pixel).

## How we use it
To record and administer sweepstakes entries, deliver order confirmations, notify winners, comply with sweepstakes law (including state registration and winners-list obligations), and — only if you opt in — send promotional email you can unsubscribe from at any time.

## What we never do
We never sell your personal data. Entry data is retained as required by sweepstakes record-keeping laws and then deleted.

## Your rights
Request a copy or deletion of your data anytime: ${site.supportEmail}. Note that entry records for an active or recently completed promotion must be retained until legal retention periods expire.`,
  },
};
